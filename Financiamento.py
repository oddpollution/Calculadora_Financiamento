    ...
    #juros = float(0.0941)
        ...
        for i in range(1,(meses+1)):
            if calculo == "PRICE":
                ...
                data=[{'Periodo': i,'Montante': total,'Parcela': total/meses,'Amortizacao': total/meses + juros_mes * tabela_price.iloc[i-1][5],'Juros': juros_mes * tabela_price.iloc[i-1][5],'Saldo_devedor': tabela_price.iloc[i-1][5] + juros_mes * tabela_price.iloc[i-1][5] - parcela}]
                data=pd.DataFrame(data)
                tabela_price = pd.concat([tabela_price,data])        
